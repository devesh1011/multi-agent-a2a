...
def generate_agent1_calendar() -> dict[str, list[str]]:
...
Agent1_CALENDAR = generate_agent1_calendar()


def get_availability(start_date: str, end_date: str) -> str:
    """
    Checks Agent1's availability for a given date range.
    ...
    Returns:
        A string listing Agent1's available times for that date range.
    """
    try:
        ...
        for i in range(delta.days + 1):
            ...
            available_slots = Agent1_CALENDAR.get(date_str, [])
            if available_slots:
                availability = f"On {date_str}, Agent1 is available at: {', '.join(available_slots)}."
...
def create_agent() -> LlmAgent:
    return LlmAgent(
        ...
        name="Karley_Agent",
        instruction="""
            **Role:** You are Agent1's personal scheduling assistant. 
            ...
            *   **Check Availability:** Use the `get_karley_availability` tool to determine 
                    if Agent1 is free on a requested date or over a range of dates. 
