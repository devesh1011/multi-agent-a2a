...
class Agent1(AgentExecutor):
    ...
    def _run_agent(
        ...
        return self.runner.run_async(
            session_id=session_id, user_id="agent-1", new_message=new_message
        )
    ...
    async def _process_request(
        ...
        async for event in self._run_agent(
            ...
            if event.is_final_response():
                ...
                task_updater.add_artifact(parts)  # type: ignore
                task_updater.complete()  # type: ignore
                break
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue):
        if not context.task_id or context.context_id:
            raise ValueError("RequestContext must have task_id and ocntenxt_id")
        if not context.message:
            raise ValueError("RequesetContext must have a message")

        updater = TaskUpdater(event_queue, context.task_id, context.context_id)  # type: ignore
        if not context.current_task:
            updater.submit()  # type: ignore
        updater.start_work()  # type: ignore
        await self._process_request(
            types.UserContent(
                parts=convert_a2a_parts_to_genai(context.message.parts),
            ),
            context.context_id,  # type: ignore
            updater,
        )
    ...
    async def _upsert_session(self, session_id: str):
        session = await self.runner.session_service.get_session(
            app_name=self.runner.app_name, user_id="agent-1", session_id=session_id
        )
        if session is None:
